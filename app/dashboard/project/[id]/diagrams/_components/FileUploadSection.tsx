...
interface FileUploadProps {
    accept?: string;
    maxSizeMB?: number;
    onFilesChange?: (files: File[]) => void;   // callback khi có file mới
    onFileRemove?: (fileId: string) => void;   // callback khi xóa file
}
...
export function FileUploadSection({
    ...
    const handleDrop = (e: React.DragEvent) => {
        ...
        if (dropped.length > 0) handleValidFiles(dropped);
    };
    ...
    const handleFileInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (filesList && filesList.length > 0) handleValidFiles(filesList);
    };

    const handleValidFiles = (fileList: FileList) => {
        const validTypes = accept.split(',');
        const maxBytes = maxSizeMB * 1024 * 1024;

        const validFiles = Array.from(fileList).filter((file) => {
            const ext = '.' + file.name.split('.').pop()?.toLowerCase();
            return validTypes.includes(ext) && file.size <= maxBytes;
        });

        const newUploaded: UploadedFile[] = validFiles.map((file) => ({
    ...
    return (
        <>
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 sm:p-8 mb-8 transition-colors duration-300">
                ...
                <div
                    className={`border-2 mt-5 cursor-pointer border-dashed rounded-lg p-12 text-center transition-colors duration-200 ${isDragOver
